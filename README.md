...
Use Ollama as the default AI provider behind an `AIProvider` interface. Route chat workloads by task (`general`, `coding`, `reasoning`) with small local defaults and documented production targets (Qwen3 32B, Qwen2.5-Coder 32B, DeepSeek-R1). Prefer JSON Schema structured outputs with validation. OpenAI, Anthropic, and other providers can be added later as adapters.
...
- Task-based model routing: general / coding / reasoning / embedding
...
- JSON Schema structured outputs, Pydantic validation, and optional JSON repair retries
- Startup warmup for the default chat model
