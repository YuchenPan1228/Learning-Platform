  ...
  return (
    <div className={panelClassName}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold tracking-wide text-[#66736e] uppercase">Draft review</p>
...
export function AdminReviewView({ initialItems }: AdminReviewViewProps) {
  ...
  async function refreshQueue(nextSelectedId?: number | null) {
    const response = await fetchReviewQueue("draft");
    setItems(response.items);
    if (nextSelectedId !== undefined) {
      setSelectedId(nextSelectedId);
      return;
    }
    if (selectedId !== null && !response.items.some((item) => item.id === selectedId)) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      <section className={panelClassName}>
        ...
        <p className="mt-1 text-sm text-[#66736e]">
          Draft extracted objects awaiting human review.
        </p>

        {items.length === 0 ? (
          <p className="mt-4 text-sm text-[#66736e]">No draft items in the queue.</p>
        ) : (
        ...
        {selectedItem ? (
          <ReviewDetailPanel
            ...
            onSaveEdit={async (qualityScore) => {
              ...
              await refreshQueue(selectedItem.id);
            }}
          />
        ) : (
          <section className={panelClassName}>
            <p className="text-sm text-[#66736e]">Select a draft item to review.</p>
          </section>
        )}
      </div>
    </div>
